...
const validatorFile = "testdata";
const relativeHeaderPath = "swagger/header.json";
const relativeOutputFile = "swagger/swagger.json";
const httpEndpoint = "https://api-feed-dev.eco.astro.com.my";
    ...
    if (joiSchema[checker]) {
      ...
      for(key in swagger.properties) {
        ...
        if (checker === 'body') {
          ...
          swaggerJson.definitions[modelName] = swagger;
          temp.schema = {
...
const constructAwsApiIntegration = (validator, parameters) => {
  const apiGateway = {
    ...
    type: "http_proxy",
    uri: httpEndpoint + validator.path,
...
const constructSwaggerResponse = (swaggerJson, validator) => {
  ...
  for(statusCode in swagger.properties) {
    ...
    swaggerJson.definitions[modelName] = swagger.properties[statusCode].properties.body;
...
const setDefaultDocumentation = (swagger) => {
  swagger.info.title = swagger.info.title + " " + process.env.NODE_ENV;
  swagger.info.description = swagger.info.description + " for " + process.env.NODE_ENV + " environment";
  swagger.paths = {};
  swagger.definitions = {};
  swagger['x-amazon-apigateway-gateway-responses'] = {
    "DEFAULT_4XX": {
      "responseParameters": {
...
const init = () => {
  glob(path.join(validatorFile, "**/*.validator.js"), function (err, files) {
    if (err) {
